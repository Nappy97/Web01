package Lec25_Thread.com.lec.java.thread02;
...
public class Thread02Main {
    ...
    public static void main(String[] args) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                for (int i = 1;i<10;i++){
                    System.out.println(Thread.currentThread().getName() + " " + i + " 덥다");
                }
            }
        }).start();
        ...
        new Thread(()->{
            for (int i = 1; i<10; i++){
                System.out.println(Thread.currentThread().getName() + " " + i + " 이것이 멀티쓰레딩");
                try {
                    Thread.sleep(100);
